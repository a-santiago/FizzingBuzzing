...
//Determine the Fizz, Buzz or FizzBuzz
//Fizz = number % input 1 = 0
//Buzz = number % input 2 = 0
//FizzBuzz = number % input 11 = - && number % input 2 =0

//print the results Fizz, Buzz, FizzBuzz or the number

//basic if/else with two posibilities and only one outcome
if (condition) {
...
// there's no limit to chains but there is a preacticality limit to it


//multiple if statements - each runs independent of eachother
if (condition3) {
...
//switches are written

switch (switchVariable) {
    case 'something': {
        alert('something was in the word');
        break;
    }
    case 'nothing': {
...
function displayData(fizzBuzzData) {
    ...
    //acces the table element
    let templateRow = document.getElementById("fbTemplate")

    //clear out any information in the table body(let's the code run without stacking)
    tableBody.innerHTML = '';

    //go through the supllied FB information and fill out the table rows with it
    for (let index = 0; index < fizzBuzzData.length; index += 5) {
        //the node below represents the tableRows in the HTML it'll access the table in HTML
        //grab the inside of the table element, but not the template element itself
...
//better fizzbuzz lets user set Fizz and Buzz numbers and loops from 1 to 100
